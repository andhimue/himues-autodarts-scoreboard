#!/usr/bin/env python3
# update.py - Standard Update-Skript für GitHub (origin) mit DB-Migration
import os
import sys
import subprocess
import shutil
...
def update_database_schema():
    ...
    # Inline-Script, um die Umgebung der venv zu nutzen
    schema_script = """
import os
import sys
import sqlite3
...
def add_column_if_missing(cursor, table, col_def):
    try:
        # Simpler Check: Versuche Spalte hinzuzufügen. Wenn Fehler -> existiert wohl schon.
        # Bei SQLite und MariaDB ist 'ADD COLUMN' valide.
        cursor.execute(f"ALTER TABLE {table} ADD COLUMN {col_def}")
        print(f"  -> Spalte 'is_win' hinzugefügt zu {table}")
    except Exception:
        # Ignorieren, wenn Spalte schon da ist
        pass
...
try:
    ...
    if conn:
        ...
        # Definition je nach DB Typ (Integer für SQLite, TinyInt für MariaDB)
...
def main():
    ...
    # 4. Datenbank Updates (Schema & Migration)
